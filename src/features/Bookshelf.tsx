...
export default class Bookshelf extends Feature {
    ...
    constructor({ name = 'bookshelf', books = [], ...params }: BookshelfDefinition) {
        super({ name, ...params });

        books?.forEach(({ type: BookType = Book, definition: bookDefinition }) => {
    ...
    public async _interaction(gameController: GameController): Promise<void> {
        ...
        gameController.console.print(<div>{this.getInteractionText(gameController)}</div>, <br />);

        const itemNodes = this._getItemNodes();
        if (itemNodes) gameController.console.print(...itemNodes, <br />, <br />);

        const featureNodes = this._getFeatureNodes();
        if (featureNodes) gameController.console.print(...featureNodes, <br />, <br />);

        let bookChoice = 0;

        if (this.books.length > 1) {
            // Multiple book flow
            gameController.console.print(...this.getBookMenu(gameController), <br />, <br />);

            const _input = await gameController.console.prompt('Pick a book by number:', {
                allowEmpty: true,
            });
            ...
            // No books flow
            await gameController.console.pause('There are no books on the shelf...');
        }

        if (bookChoice === -1) {
            await gameController.console.pause("You don't feel like reading right now.");
            ...
            await gameController.console.pause("You're not sure how you'd do that...");
    ...
    private getBookMenu(gameController: GameController) {
        ...
        return [
            ...this.books.map((book, idx) => {
                ...
                const bookLabel = padIndex ? bookNumStr.padStart(2, ' ') : bookNumStr;
